...
export default async function decorate(block) {
  ...
  // Check if first row is an intro/header (has heading but no image)
  const firstRow = rows[0];
  const hasHeadingNoImage = firstRow
    && firstRow.querySelector('h1, h2, h3')
    && !firstRow.querySelector('picture, img');

  let slideRows = [...rows];
  if (hasHeadingNoImage) {
    // Extract first row as header, not a slide
    const header = document.createElement('div');
    header.classList.add('carousel-products-header');
    while (firstRow.firstElementChild) header.append(firstRow.firstElementChild);
    firstRow.remove();
    block.prepend(header);
    slideRows = slideRows.slice(1);
  }
